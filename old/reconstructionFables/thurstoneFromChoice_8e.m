...
modelDir = './';
modelName = 'thurstoneFromChoice_8e';
...
for dataIdx = 1:numel(dataList)
   ...
   switch dataName
      ...
      case 'tomicBaysMemorySet3'
         dataDir = 'data/';
         dataName = 'tomicBays';
         load([dataDir dataName], 'dm');
         dataName = 'tomicMemorySet3';

         keep = find(dm.setSize == 3);

         y = dm.response(keep);
         nTrials = length(keep);
         nStimuli = dm.nStimuli;
         stim = [dm.tIdx(keep) dm.nIdx(keep, 1:2)];
         setSize = 3;
      ...
      case 'tomicBaysMemorySet6'
         dataDir = 'data/';
         dataName = 'tomicBays';
         load([dataDir dataName], 'dm');
         dataName = 'tomicMemorySet6';

         keep = find(dm.setSize == 6);

         y = dm.response(keep);
         nTrials = length(keep);
         nStimuli = dm.nStimuli;
         stim = [dm.tIdx(keep) dm.nIdx(keep, :)];
         setSize = 6;
   ...
   omega = codatable(chains, 'omega', @mean)
