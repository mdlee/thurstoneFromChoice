...
modelDir = './';
modelName = 'thurstoneFromChoice_5g';


dataList = {...
   %'whoIsSimilar_A'; ...
   'tomicBays'; ...
   % 'tomicBaysLimitedStimuli'; ...
...
for dataIdx = 1:numel(dataList)
   ...
   switch dataName
      ...
      case 'tomicBaysLimitedStimuli'
         dataDir = 'data/';
         dataName = 'tomicBays';
         load([dataDir dataName], 'ds');
         dataName = 'tomicBaysLimitedStimuli';

         keep = 1:2:72;

         match = find(ismember(ds.aIdx, keep) & ismember(ds.bIdx, keep) & ismember(ds.cIdx, keep) & ismember(ds.dIdx, keep));
         ...
         d = ds.dIdx(match);
         y = ds.response(match);
         nTrials = length(match);
         nStimuli = length(keep);
      ...
      case 'tomicBaysLimitedTrials'

         dataDir = 'data/';
         dataName = 'tomicBays';
         load([dataDir dataName], 'ds');
         dataName = 'tomicBaysLimitedTrials';

         tmp = randperm(ds.nTrials);
         ds.nTrials = 2e3;
         keepTrials = tmp(1:ds.nTrials);


         a = ds.aIdx(keepTrials);
         b = ds.bIdx(keepTrials);
         c = ds.cIdx(keepTrials);
         d = ds.dIdx(keepTrials);
         y = ds.response(keepTrials);
         nTrials = ds.nTrials;
         nStimuli = ds.nStimuli;
   ...
   generator = @()struct('xA', xAinit, 'xB', xBinit, ...
      'xC', xCinit, 'xD', xDinit, 'mu', ds.stimuli);
      ...
      switch analysis
         case 'recoveryScatter'
            ...
            set(gca, ...
               ...
               'xtick'      , [0 pi/2 pi]   , ...
               'xticklabelrot', 0, ...
               'xticklabel' , {'$0$' , '$\frac{\pi}{2}$', '$\pi$'}, ...
               'ylim'       , [0 pi]    , ...
               'ytick'      , [0 pi/ 2 pi]  , ...
               'yticklabel' , {'$0$' , '$\frac{\pi}{2}$', '$\pi$'}, ...
